...
def send_to_back(shape, slide):
    """Pushes connector lines BEHIND the boxes"""
...
def index():
    return "Org Chart API is running! Send a POST request with an Excel file to /generate", 200

@app.route('/generate', methods=['POST'])
def generate_org_chart():
    try:
        if 'file' not in request.files:
            return jsonify({"error": "No file uploaded. Please upload an Excel file."}), 400
        
        file = request.files['file']
        if file.filename == '':
            return jsonify({"error": "Empty filename."}), 400

        # READ FILE DIRECTLY FROM MEMORY (No saving to disk)
        ...
        for _, row in df.iterrows():
            ...
            if name in managers:
                nodes[name] = {'title': row['Title'], 'subtitle': name, 'color': COLOR_MAP.get(row['Label'], COLOR_MAP['None']), 'is_count': False}
        ...
        for idx, row in grouped.iterrows():
            node_id = f"LEAF_{idx}"
            nodes[node_id] = {'title': row['Title'], 'subtitle': str(row['Count']), 'color': COLOR_MAP.get(row['Label'], COLOR_MAP['None']), 'is_count': True}
            manager = row['Reports to']
            dept = str(row['Department'])
            if manager == ceo_name and dept in depts:
        ...
        # ==========================================
        # THE FIX: IN-MEMORY SAVING (Vercel Safe)
        # ==========================================
        ppt_stream = io.BytesIO()
        prs.save(ppt_stream)
        ppt_stream.seek(0)

        # Send file directly as a download response
        return send_file(
            ppt_stream,
            as_attachment=True,
            download_name="Professional_OrgChart.pptx",
